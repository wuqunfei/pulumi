...
func update(ctx *Context, info *planContext, opts planOptions, dryRun bool) (Plan, ResourceChanges, result.Result) {
	...
	planner, err := newPlanner(ctx, info, opts, dryRun)
	if err != nil {
		return nil, nil, result.FromError(err)
	}
	defer contract.IgnoreClose(planner)

	return planner.run(ctx, actions, policies, dryRun)
}
